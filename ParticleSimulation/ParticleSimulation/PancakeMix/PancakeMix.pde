...
class Spheres{
  ...
  boolean goOut;
  float alpha;
  public Spheres(float radius, color c, float[] position, float[] velocity, float lifeSpan, boolean fall, boolean goOut, float alpha, boolean spawned){
    this.radius = radius;
    this.c = c;
...
void computePhysics(float dt, Spheres curSphere){
  ...
  if (distance < 35 && abs(curSphere.position[1] - floor) <= radius){
    ...
    curSphere.spawned = false;
    curSphere.c =color(205,133,63);
  ...
  if (curSphere.fall){
     ...
     if (( (int) abs(curSphere.position[1]) % 10 == 3 || (int) abs(curSphere.position[1]) % 10 == 4 || (int) abs(curSphere.position[1]) % 10 == 5) && !curSphere.spawned ){
    curSphere.c = color(255,248,220);
  }
  else if (!curSphere.spawned) {curSphere.c = color ( 205,133,63);}
...
void spawnParticles(float dt){
  ...
  for (int i = 0; i < numParticles; i++){
     ...
     float[] vel = {2.2*random(-1, 1), -10, 2.2*random(-1,1)};
     color c = color(255, 255, 153);
     //color should be based off position
     //life span?
     Spheres newSphere = new Spheres(radius, c, pos, vel, random(0.5,1), true, false, 255, true);
